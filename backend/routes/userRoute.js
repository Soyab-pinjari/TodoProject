...
try {
    ...
    const hashdPassword= await bcrypt.hash(password,10)  //TO create encrypted password 10 is a value of hardness it can be 20,20 more..
...
router.post('/login',async (req,res)=>{
   try {
    ...
    const isMatch = await bcrypt.compare(password,user.password);
     if(!isMatch) return res.status(400).json({message:'invalid cradintial'});
